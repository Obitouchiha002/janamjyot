...
export function buildPanchang(input: PanchangInput) {
  ...
  if (sunrise && sunset) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      dayChoghadiya.push({ name, start: fmtTime(s, timezone), end: fmtTime(e, timezone), quality: CHO_GOOD.has(name) ? "good" : CHO_BAD.has(name) ? "bad" : "neutral" });
    }
    const nextSunrise = sunRiseSet(sunset, latitude, longitude).sunrise;
    if (nextSunrise) {
      const nightPart = (nextSunrise.getTime() - sunset.getTime()) / 8;
      for (let i = 0; i < 8; i++) {
        const name = CHO_CYCLE[(CHO_NIGHT_START[dow] + i) % 7];
        const s = new Date(sunset.getTime() + i * nightPart);
        const e = new Date(s.getTime() + nightPart);
        nightChoghadiya.push({ name, start: fmtTime(s, timezone), end: fmtTime(e, timezone), quality: CHO_GOOD.has(name) ? "good" : CHO_BAD.has(name) ? "bad" : "neutral" });
      }
    }
  }
